...
A browser-based dashboard is served at `/pth/dashboard` on the same DigitalOcean Droplet. The interface is built in vanilla HTML, CSS, and JavaScript, using Chart.js for visualization. On load, it fetches the past 7 days of data from `/pth/api/range` and renders a time-series line plot — or, if the URL includes `start`/`end` query parameters (e.g. `/pth/dashboard?start=1740000000&end=1742000000`), it loads that range instead, making it easy to share a link straight to a specific window of data. Each sensor channel (pressure, temperature, humidity) is assigned its own Y-axis, allowing channels with disparate units and scales to be displayed simultaneously without compression.
